...
is stored as a real-valued "S" object under the hood in C. `REF(2)` indicates that only two variables are referring to this particular memory location (more on this in much detail in a bit).
...
can give us confusing results for `ref()`, so I'm adding the clean results from just
running in R here in comments.
