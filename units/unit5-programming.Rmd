...
> 2022-09-23: Currently only Sections 1-9 are available.
...
with a given bucket. So each bucket will contain a list of a small number of values.
...
Some things to think about in trying to write more efficient code (or improve the efficiency of existing code) in R and more generally include:
  ...
  - avoid repeated calculations, particularly in a loop
     ...
     - move repeated calculations outside of loops (watch out for calculations in a loop that don't involve all the indices of the loop(s))
  - avoid entirely unneeded calculations (e.g., consider `diag(X%*%Y)`)
...
$n=2000$, $p=0.3$ and $\phi=0.5$ when you need to actually run your
code.
