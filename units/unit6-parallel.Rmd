...
-   *threads*: multiple paths of execution within a single process; the
    OS sees the threads as a single process, but one can think of them
...
While there are various R packages (e.g., `Rmpi` and the `pbdR`
...
|      remote    |    uses an R process on another machine    |     yes     |             yes |
...
`future.scheduling` or `future.chunk.size` arguments.
...
distributed datasets in Unit 8 but here we'll introduce the basic
functionality.
...
This is the analog of apply/lapply/sapply type functions in R. The term
'map' here is a functional programming term (as well as having the same
meaning as 'map' in the context of MapReduce).
