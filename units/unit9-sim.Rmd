...
-   If we have a *method for finding a confidence interval*, what
...
$X$, where
$\hat{\beta}=(X^{\top}X)^{-1}X^{\top}Y$. If we know that we have
...
homoscedastic errors. Or suppose we have a modified procedure to produce
...
Note that in most simulation studies, $Y$ is an entire dataset, and the "iid
...
we want to know: $$\phi=E\hat{\beta}.$$ We can use the Monte Carlo
estimate of $\phi$:
...
coverage as
$$\hat{\phi}=\frac{1}{m}\sum_{i=1}^{m}1_{\beta\in CI(y_{i})}.$$ Of
course we want that $\hat{\phi}\approx1-\alpha$ for a $100(1-\alpha)$
